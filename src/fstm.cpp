...
Fstm::Fstm(std::span<const u8> data) {
  ...
  for (auto& ref : header.block_refs) {
    ...
    case ElementType::StreamSoundFile_DataBlock:
      ...
      for (auto& channel : m_samples) {
        channel = reader.ReadSamples(m_info.StreamInfo().frame_count, m_info.StreamInfo().encoding,
                                     false);
        reader.Align(0x20);
      }
      break;
...
std::vector<u8> Fstm::ToBinary(util::Endianness endian) const {
...
void Fstm::Serialize(util::AudioWriter& writer) const {
  ...
  // REGN
  if (m_has_region) {
    writer.WriteCurrentOffsetAt<std::int32_t>(pending_header_values["blocks"][2], file_start);
  ...
  // DATA
  writer.WriteCurrentOffsetAt<std::int32_t>(pending_header_values["blocks"][3], file_start);
...
void InfoBlock::Serialize(util::AudioWriter& writer) const {
  ...
  std::size_t strm_info_offset_pos{
      writer.WriteEmptyOffsetReference(ElementType::StreamSoundFile_StreamSoundInfo, true)};

  std::size_t track_info_ref_table_offset_pos =
      writer.WriteEmptyOffsetReference(ElementType::Table_ReferenceTable, !m_track_infos.empty());
  ...
  if (!m_track_infos.empty()) {
    writer.WriteCurrentOffsetAt<std::int32_t>(track_info_ref_table_offset_pos,
    ...
    for (uint i{0}; i < m_track_infos.size(); ++i) {
      track_infos_offset_pos[i] =
          writer.WriteEmptyOffsetReference(ElementType::StreamSoundFile_TrackInfo, true);
    }
  ...
  if (!m_detail_channel_infos.empty()) {
    writer.WriteCurrentOffsetAt<std::int32_t>(channel_info_ref_table_offset_pos,
                                              reference_array_start);
    ...
    for (uint i{0}; i < m_detail_channel_infos.size(); ++i) {
      channel_infos_offset_pos[i] =
          writer.WriteEmptyOffsetReference(ElementType::StreamSoundFile_ChannelInfo, true);
    }
    ...
    for (uint i{0}; i < m_detail_channel_infos.size(); ++i) {
      writer.WriteCurrentOffsetAt<std::int32_t>(detail_channel_infos_offset_pos[i],
                                                detail_channel_infos_offset_pos[i] -
                                                    sizeof(std::uint32_t));
