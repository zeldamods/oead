...
InfoBlock::InfoBlock(util::AudioReader& reader) {
  ...
  if (track_info_table_ref.offset != -1) {
    ...
    for (u32 i{0}; i < track_info_table.count; ++i) {
  ...
  if (channel_info_table_ref.offset != -1) {
    ...
    for (u32 i{0}; i < channel_info_table.count; ++i) {
...
void Fstm::SerializeDataBlock(util::AudioWriter& writer) const {
  ...
  writer.WriteCurrentOffsetAt<s32>(section_size_offset, section_start);
}
...
void InfoBlock::Serialize(util::AudioWriter& writer) const {
  ...
  writer.WriteCurrentOffsetAt<s32>(strm_info_offset_pos, reference_array_start);
  ...
  if (!m_track_infos.empty()) {
    ...
    for (u32 i{0}; i < m_track_infos.size(); ++i) {
      writer.WriteCurrentOffsetAt<s32>(track_infos_offset_pos[i], track_info_table_start);
